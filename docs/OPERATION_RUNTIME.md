...
Online Tools Hub v1.0 把当时十二个工具的纯核心包装为同一套可组合执行单元。v1.1 新增第十三个 `regex.test` 与第十四个 `qr.transform` Operation，并为正则和二维码工具页保留更严格的专属 Worker 隔离。Operation Runtime 只负责本地计算，不负责界面状态、正文持久化、网络请求或工作流编排。
...
- `src/workers/operation.worker.ts` 与二维码专属 `qr-operation.worker.ts` 在独立执行域内再次校验请求与输出；后者把 `uqr`/`jsQR` 闭包从公共 Worker 中隔离，使每个生产脚本继续低于 100 KiB。

纯 catalog 与 runtime 分离是强制边界。首页、内容页以及单工具页不会因为列出 Operation 而下载其他工具实现。

正则与二维码工具页进一步不导入 `OperationExecutor`：它们通过轻量客户端为每次任务直接创建专属一次性 Worker。`regex.test` 与 `qr.transform` adapter 仍在 runtime registry 中按需提供给工作流等组合入口，两条路径共享各自纯核心和数据契约，但工具页面不会因此带入中央执行器、其他 adapter 或主线程算法回退。工作流执行 `qr.transform` 时由 executor 选择二维码专属 Operation Worker；其余十三项继续使用公共 Worker，两个入口共享同一严格协议与输出复核实现。
...
正则测试的 pattern 复杂度不可预测，因此无论输入多小都必须进入独占 Worker。页面取消、`pagehide`、Worker 异常或 2 秒硬超时走同一终止与结算路径；超时后不得尝试在主线程重跑。pattern、测试文本、匹配项、捕获与序列化输出还分别受固定预算限制。

二维码生成和识别也始终进入一次性 Worker，8 秒超时后直接终止。生成路径只返回固定几何 SVG；识别路径只接收已通过工具页头部验证并缩至 4 MP 的 RGBA，Operation 路径同样复核尺寸、像素、内存与输出预算。识别文本不获得导航、预取或网络能力。
...
- v1.0 基线的十二个 manifest 与十二个动态 adapter 一一对应；当前 catalog 的十四个 manifest 与十四个动态 adapter 继续一一对应；
...
- 正则工具页不导入中央执行器，专属 Worker 与 `regex.test` Operation 路径都通过灾难性回溯、取消、超时、零宽 Unicode 匹配和输出预算测试；
- 二维码工具页不导入中央执行器，专属 Worker 与 `qr.transform` Operation 路径都通过 Unicode 往返、固定 SVG、旋转/反色/低清图片、取消、超时和纯文本结果测试；
