...
Online Tools Hub v1.0 把当时十二个工具的纯核心包装为同一套可组合执行单元。v1.1 新增第十三个 `regex.test` Operation，并为正则工具页保留更严格的专属 Worker 隔离。Operation Runtime 只负责本地计算，不负责界面状态、正文持久化、网络请求或工作流编排。
...
正则工具页进一步不导入 `OperationExecutor`：它通过轻量客户端为每次测试直接创建专属一次性 Worker。`regex.test` adapter 仍在 runtime registry 中按需提供给工作流等组合入口，两条路径共享同一纯核心和数据契约，但工具页面不会因此带入中央执行器、其他 adapter 或主线程回退。
...
- v1.0 基线的十二个 manifest 与十二个动态 adapter 一一对应；当前 catalog 的十三个 manifest 与十三个动态 adapter 继续一一对应；
