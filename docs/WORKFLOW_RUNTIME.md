...
Workflow Runtime 在 Operation Runtime 之上提供线性、内存内的数据处理流水线。它只负责编译和执行配方；面向用户的 Workflow Studio、文件解码、批处理和下载体验由后续阶段接入。
...
第二阶段由 `runner.ts` 串行调度。Runner 同时只允许一个活动运行，按已编译计划把 Vault 中的防御性副本交给 OperationExecutor，并把每一步结果重新存入 Vault。执行快照只暴露步骤 ID、状态、错误码和 opaque payload ID，不包含正文。
...
模板、配方和 notices 均深冻结。它们只组合已有白名单 Operation，不包含远程 URL、动态模块或用户脚本。#37 的图片模板从已验证 RGBA 开始；真实文件解码、完整图片参数和批处理属于 #35。
...
- 每次只运行一个 workflow，批处理与逐项失败隔离由 #35 增加；
...
- 当前底层 runtime 不等于完整 Studio UI，也不代表所有本地文件格式都已接入。
