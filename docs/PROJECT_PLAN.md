...
路线图中的 Phase 2、Phase 3 只是候选范围。只有进入具体里程碑的功能才视为已承诺开发。
...
MVP 上限为 5 个核心工具。在全部通过验收前，不增加第 6 个工具。
...
| 文件输入 | MVP 暂不开放 | Phase 2 单独评估内存占用 |
...
- 最新两个主要版本的 Chrome、Edge、Firefox、Safari 通过核心流程冒烟测试。
...
| Ready PR | Draft 检查 + Playwright 核心流程 |
| `main` | 全量检查 + 静态部署 |
| 手动预检 | Playwright 多浏览器 + Lighthouse |
...
完成条件：全部通过第 12 节验收，且连续 14 天没有未解决的 P0/P1 问题。
...
| GitHub Pages 路径差异 | 刷新或资源路径 404 | CI 中验证 `basePath`、静态路由和部署 URL |
...
- 首版严格限制为 JSON、Base64、URL、时间戳、UUID 五个工具；
