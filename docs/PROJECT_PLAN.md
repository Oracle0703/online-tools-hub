...
- 已提供 10 个稳定工具：5 个 MVP 文本工具、0.4.0 图片工具，以及 0.7.0 新增的 4 个开发工具；
...
MVP 的 5 个核心文本工具已经完成验收，0.4.0 依照新工具准入规则加入图片压缩与格式转换。0.7.0 继续加入文本差异、SHA 哈希、YAML / JSON 互转和 JWT 声明检查，使当前范围达到 10 个工具，同时保持无后端依赖和本地优先。
...
当前十个工具的处理流程均不经过应用服务器。构建和部署可以联网，用户处理内容时不需要联网。
...
启动条件：当前十个工具稳定，并且候选工具获得至少 3 条独立用户需求或 Issue 投票。
...
- 首版完成 JSON、Base64、URL、时间戳、UUID 五个 MVP 工具；0.4.0 加入本地图片压缩，0.7.0 加入文本差异、SHA 哈希、YAML / JSON 互转和 JWT 声明检查，当前共 10 个工具；
