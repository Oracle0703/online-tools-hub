...
当前十二个工具的处理流程均不经过应用服务器。构建和部署可以联网；PWA 缓存只包含公开静态资源，用户处理内容时不需要联网。
...
- PWA 只缓存构建期公开资源，离线缓存中不得出现用户内容或运行期请求；
...
#35 的公开信息架构使用 `/workflows/` 和六个模板 slug。静态 Astro 页面负责可发现内容、输入/结果边界、HowTo 与 Breadcrumb SEO；React Workflow Studio 只负责纵向步骤编辑和当前标签页运行态。Header、Footer、全站搜索、sitemap、Service Worker 预缓存和构建直达路由共同暴露公开页面；`/__runtime/workflows/` 仍只用于自动验收并保持 noindex、无 canonical。文件和批处理必须通过显式输入组件产生已验证 payload，不能扩充 recipe wire format；公开 UI 一次最多 12 项、合计 64 MiB，串行执行并逐项隔离失败，ZIP 和不含文件名/正文/哈希的回执只在用户主动操作时生成。
