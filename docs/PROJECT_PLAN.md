...
- 已提供 12 个稳定工具：5 个 MVP 文本工具、图片工具、4 个开发工具，以及 0.8.0 新增的 2 个数据工具；
...
MVP 的 5 个核心文本工具已经完成验收，0.4.0 依照新工具准入规则加入图片压缩与格式转换。0.7.0 继续加入文本差异、SHA 哈希、YAML / JSON 互转和 JWT 声明检查，0.8.0 再加入 CSV / JSON 与查询参数工具，使当前范围达到 12 个工具。0.9.0 不增加工具数量，而是用本地智能识别、全站命令面板与安全跨工具接力缩短任务路径，同时保持无后端依赖和本地优先。
...
当前十二个工具的处理流程均不经过应用服务器。构建和部署可以联网；PWA 缓存只包含经过当前构建清单、字节数与 SHA-256 校验的公开静态资源，用户处理内容时不需要联网。安装阶段只保存最小应用壳，完整工具与工作流离线包由用户主动下载，并可查看容量、取消、继续或删除。
...
- `/privacy-manifest.json` 必须由 12 个工具、12 个 Operation 和 6 个工作流注册表生成，缺失、无效或 CSP 不满足时构建失败；
...
- [ ] 机器隐私清单覆盖 12 个工具、12 个 Operation 和 6 个工作流，隐私自检准确说明“仅观察本站当前标签页本次合成执行”；
...
- 首版完成 JSON、Base64、URL、时间戳、UUID 五个 MVP 工具；0.4.0 加入本地图片压缩，0.7.0 加入文本差异、SHA 哈希、YAML / JSON 互转和 JWT 声明检查，0.8.0 加入 CSV / JSON 与查询参数工具，当前共 12 个工具；0.9.0 用智能识别、统一搜索与安全接力连接现有能力；
