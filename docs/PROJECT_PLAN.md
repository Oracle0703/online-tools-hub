...
MVP 的 5 个核心文本工具已经完成验收，0.4.0 依照新工具准入规则加入图片压缩与格式转换。0.7.0 继续加入文本差异、SHA 哈希、YAML / JSON 互转和 JWT 声明检查，0.8.0 再加入 CSV / JSON 与查询参数工具，使当前范围达到 12 个工具，同时保持无后端依赖和本地优先。
...
- 首版完成 JSON、Base64、URL、时间戳、UUID 五个 MVP 工具；0.4.0 加入本地图片压缩，0.7.0 加入文本差异、SHA 哈希、YAML / JSON 互转和 JWT 声明检查，0.8.0 加入 CSV / JSON 与查询参数工具，当前共 12 个工具；
