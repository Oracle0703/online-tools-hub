...
export const guides = [
  {
    slug: "qr-code-local-scan-safety",
    title: "二维码内容不是安全链接：本地生成、识别与点击边界",
    summary:
      "二维码只编码数据，不证明链接、付款对象或发送者可信；先本地识别为纯文本，再决定是否访问。",
    description:
      "解释二维码生成与图片识别的本地处理边界，以及面对未知网址、伪装域名和打印损伤时的安全检查方法。",
    eyebrow: "二维码安全",
    mark: "QR",
    readingMinutes: 3,
    ...
    relatedToolSlugs: ["qr-code"],
    relatedWorkflowSlugs: [],
    sections: [
      {
        id: "data-not-trust",
        title: "二维码只是数据载体，不是可信证明",
        paragraphs: [
          "二维码可以保存普通文本、网址、联系方式或应用自定义指令。图案能被正确识别，只说明其中的数据符合编码规则，并不证明域名属于预期组织、付款对象正确，或发送者拥有可信身份。",
          "攻击者可以把相似域名、重定向链接或让人误判的字符放进二维码。贴纸也可能覆盖原有标牌，因此现实位置看起来可信，图案里的内容仍需要单独核对。",
        ],
        callout:
          "把“识别内容”和“执行内容”分成两个动作：先看完整纯文本，再决定是否复制、访问或交给其他应用。",
      },
      {
        id: "scan-without-opening",
        title: "识别时不要自动访问结果",
        paragraphs: [
          "更安全的识别流程只读取本地 JPEG、PNG 或 WebP，把结果放进不可执行的纯文本区域。页面不应把内容渲染成链接，也不应导航、预取、验证或请求识别出的地址，因为这些行为都会在用户判断前接触目标系统。",
          "看到 https: 也不能直接下结论。应展开并核对完整域名、协议、端口、路径和混淆字符；javascript:、自定义协议、缩短网址与要求立即登录或付款的内容尤其需要停止处理。",
        ],
        points: [
          "确认完整域名，而不只看品牌词或路径；",
          "通过独立可信渠道确认发送者与预期用途；",
          "不要在未知页面输入密码、验证码或付款信息；",
          "识别失败时换清晰原图，不要反复裁掉二维码白边。",
        ],
      },
      {
        id: "generate-locally",
        title: "本地生成仍要考虑内容与可读性",
        paragraphs: [
          "本地生成可以避免把原文交给短链接或二维码服务，但图像本身仍会公开承载内容。访问令牌、恢复码、个人信息和长期有效的内部地址不应仅因为二维码难以肉眼阅读就被当作已经隐藏。",
          "纠错级别越高，二维码通常越密，但能容忍更多轻微污损。应保留足够安静区，按最终屏幕或印刷尺寸测试，并在分享前确认生成内容与预期完全一致。",
          "中文与 Emoji 按现代扫码器普遍采用的 UTF-8 字节约定编码；若二维码要交给老旧或受管设备使用，仍应在目标扫码器上做一次实际兼容测试。",
        ],
        callout:
          "二维码提供便捷传递，不提供加密、身份验证或网址信誉判断；敏感数据仍应使用经过审计的安全传输方式。",
      },
    ],
  },
  {
